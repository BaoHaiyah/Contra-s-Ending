...
void CGame::Draw(float x, float y, LPTEXTURE tex, RECT* rect)
{
	...
	if (GetKeyState(VK_LEFT) & 0x8000) isLeft = true;
	if (GetKeyState(VK_RIGHT) & 0x8000) isLeft = false;
	...
	{
		if (isLeft) sprite.TexCoord.x = rect->right / (float)tex->getWidth();
		else sprite.TexCoord.x = rect->left / (float)tex->getWidth();
		...
		if (isLeft) sprite.TexSize.x = -sprite.TexSize.x;
	}
